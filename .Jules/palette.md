...
## 2024-05-24 - Confirmation Dialogs for Destructive Actions in Lists
**Learning:** In SwiftUI applications, placing destructive or disruptive actions (such as 'Log Out' or 'Delete') directly as buttons within scrollable `List` components can easily lead to unintended execution from accidental mistaps or errant scrolling gestures.
**Action:** Always wrap destructive or disruptive actions presented as buttons in scrollable lists within a `.confirmationDialog` or `.alert` to enforce user intent and prevent unintended execution.
