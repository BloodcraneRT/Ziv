...
## 2024-05-24 - Destructive Action Confirmations
**Learning:** Destructive or session-ending actions (like "Log Out") placed inside scrollable SwiftUI `List` components are prone to accidental mistaps, which can significantly disrupt the user experience.
**Action:** Always wrap destructive or disruptive actions presented as buttons in scrollable lists within a `.confirmationDialog` or `.alert` to require user confirmation before execution.
